...
def _show_model_section(config: Dict[str, Any]) -> None:
    _section("Model")
    # `model` may be a bare string (legacy/fresh config) or a dict in the modern shape
    # ({"default": ..., "provider": ...}, see normalization at the resolve_* sites). Render the
    # name, not the raw dict repr; values go through redact_config_value so secret-shaped content
    # stays masked.
    model_val = config.get('model')
    if isinstance(model_val, dict):
        name = str(redact_config_value(model_val.get('default') or model_val.get('model') or '')).strip()
        provider = (model_val.get('provider') or '').strip()
        model_display = (f"{name} ({provider})" if provider else name) if name else color('(not set)', Colors.DIM)
    elif isinstance(model_val, str) and model_val.strip():
        model_display = str(redact_config_value(model_val.strip()))
    else:
        model_display = color('(not set)', Colors.DIM)
