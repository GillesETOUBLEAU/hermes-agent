...
def start_server(
    ...
    config = uvicorn.Config(
        ...
        #    which fails closed on "*" and /0 networks.  Explicit config wins
        #    in both modes.
        #  - ungated, unconfigured -> uvicorn's loopback-only default.
        #  - gated, unconfigured (fork patch) -> trust every peer.
        # Why the last one: proxy_headers alone is not enough, since uvicorn
        # only honours X-Forwarded-* when the direct peer is in
        # forwarded_allow_ips.  Railway's (and Fly's) TLS terminator reaches
        # the container from a private network IP that is neither loopback nor
        # a stable CIDR we could pin in config, so with the bounded list the
        # forwarded scheme is ignored, detect_https() sees "http", and session
        # cookies are minted without the Secure flag.  In gated mode the
        # container is only reachable through that terminator, so trusting
        # every peer is safe; narrow it with dashboard.trusted_proxies or
        # FORWARDED_ALLOW_IPS.
