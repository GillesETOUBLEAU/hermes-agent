...
def check_respawn_guard(
    ...
    (PR URL in a recent comment; re-spawning risks a duplicate PR — likewise
    bypassed by a re-queue event newer than the PR comment, but WITHOUT
    ``reclaimed``: an automatic crash reclaim right after a worker opened a PR is
    the exact duplicate-PR scenario this guard exists to prevent). The review
    ...
    #    Exception: an explicit operator re-queue AFTER the newest PR comment (a
    #    status drag, a dependency re-promotion, an unblock) is a deliberate "run
    #    it again" — honor it, mirroring the recent_success bypass above. Without
    #    this, the canonical block(needs_input, PR: …) → operator unblock handoff
    #    leaves the task parked in ready for up to 24h: the unblock lifts
    #    recent_success but the PR comment re-defers every tick. ``reclaimed`` is
    #    deliberately NOT in the bypass set here — see the docstring.
