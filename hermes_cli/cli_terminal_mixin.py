...
class CLITerminalMixin:
    ...
    def _schedule_focus_regain_redraw(self, min_interval: float = 1.0) -> None:
        ...
        """
        now = time.monotonic()
        # ``None``, not 0.0, for "never fired yet". time.monotonic() counts from boot on
        # Linux, so on a freshly started machine `now` is itself a small number — against a
        # 0.0 sentinel the very first focus regain looks like it happened `now` seconds ago
        # and gets rate limited away. Sub-second at the 1.0s default, but it is the wrong
        # answer, and it made the 60s-interval test fail whenever CI ran it within a minute
        # of the runner booting.
