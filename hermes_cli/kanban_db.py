...
def check_respawn_guard(
    ...
    ``"active_pr"``
        ...
        Bypassed when an explicit operator re-queue event (status change,
        promote, unblock) arrives AFTER the newest PR comment — same
        deliberate-re-run semantics as ``recent_success``, minus
        ``reclaimed``: an automatic crash reclaim right after a worker
        opened a PR is the exact duplicate-PR scenario this guard exists
        to prevent, so it must not lift the guard.
    ...
    #    Exception: an explicit operator re-queue AFTER the newest PR comment
    #    (a status drag, a dependency re-promotion, an unblock) is a deliberate
    #    "run it again" — honor it, mirroring the recent_success bypass above.
    #    Without this, the canonical block(needs_input, PR: …) → operator
    #    unblock handoff leaves the task parked in ready for up to 24h: the
    #    unblock lifts recent_success but the PR comment re-defers every tick.
    #    ``reclaimed`` is deliberately NOT in the bypass set here (unlike
    #    recent_success): an automatic crash reclaim right after a worker
    #    opened a PR is the exact duplicate-PR scenario this guard prevents.
    ...
    for c in conn.execute(
        ...
        if c["body"] and _RESPAWN_GUARD_PR_URL_RE.search(c["body"]):
            created = int(c["created_at"] or 0)
            if latest_pr_comment_at is None or created > latest_pr_comment_at:
                latest_pr_comment_at = created
    if latest_pr_comment_at is not None:
        requeued_after = conn.execute(
            "SELECT 1 FROM task_events "
            "WHERE task_id = ? AND created_at >= ? "
            "AND kind IN ('status', 'promoted', 'unblocked') "
            "LIMIT 1",
