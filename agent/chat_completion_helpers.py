...
from agent.message_sanitization import (
    _sanitize_surrogates, _repair_tool_call_arguments, tool_args_look_truncated)
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _assemble_tool_calls(tool_calls_acc, finish_reason):
        ...
        Third return value carries the raw payload of every tool call whose arguments could
        not be parsed or repaired, so the caller can tell a cut-off stream apart from a
        ...
        return mock_tool_calls or None, has_truncated_tool_args, broken_arg_payloads
    ...
    def _finish_chat_stream(self, stream, role, content_parts, reasoning_parts, tool_calls_acc, finish_reason,
        ...
        mock_tool_calls, has_truncated_tool_args, broken_arg_payloads = (
            self._assemble_tool_calls(tool_calls_acc, finish_reason))
        ...
        if has_truncated_tool_args:
            # Unparseable arguments are not automatically a TRUNCATION. Stamp "length" only
            # when the model really did run out of room: the provider said so, or the payload
            # stops mid-value (routers do rewrite "length" to "tool_calls", hiding a real
            # cut-off). A payload that closes cleanly but is still invalid JSON is a provider
            # corrupting its own tool call — z-ai/glm-5.2 via OpenRouter emitted
            # `"kind": needss_input"` with ~200 output tokens against a 131_072 cap. Calling
            # that "max output tokens" sends the operator hunting a budget problem that does
            # not exist, and the loop's four retries at double the budget cannot help. Leave
            # the provider's finish_reason so the call reaches dispatch, where invalid
            # arguments earn an honest tool error the model can act on.
            if finish_reason == "length" or any(
                    tool_args_look_truncated(payload) for payload in broken_arg_payloads):
                effective_finish_reason = "length"
            else:
                logger.warning(
                    "Provider returned complete but unparseable tool_call arguments "
                    "(finish_reason=%r, tools=%s); routing to the invalid-arguments path "
                    "instead of the truncation handler.",
                    finish_reason,
