...
| 3 | **Site repository** `WMH-Project/Xpeng-Global-Training` (private) — MCP `github`, `get_file_contents`, branch `main` | `src/config/event.ts` (facts), `src/content/en.ts` (every text of the site), `src/lib/email/types.ts` (email params) | The authoritative wording. Use it when the facts sheet is silent or you doubt it, and during the daily facts refresh |
| 4 | **Google Drive folder** of the project (id in `XPENG_DRIVE_FOLDER_ID`) — skill `google-workspace`, `google_api.py drive search/download` | documents shared by the client/logistics (agenda, hotel, briefs) | Details that never reach the site (agenda by group, hotel contract, shuttle plan) |
...
1. `get_file_contents` on `src/config/event.ts` and `src/content/en.ts` (sections `landing`,
   `programme`, `practicalInfo`, `travel`, `register`), and `select key, value from
   public.app_settings`. If `XPENG_DRIVE_FOLDER_ID` is set: `google_api.py drive search
   "'<id>' in parents"` and download any file newer than the last refresh.
